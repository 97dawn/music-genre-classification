...
based_model_last_block_layer_number = 23  # value is based on based model selected.
BATCH_SIZE = 16  # try 4, 8, 16, 32, 64, 128, 256 dependent on CPU/GPU memory capacity (powers of 2 values).
nb_epoch = 8 # number of iteration the algorithm gets trained.
...
def xception():
    ...
    model.add(Dense(512, name='dense_1', kernel_regularizer=regularizers.l2(L2_LAMBDA)))
...
if __name__ == '__main__':
    ...
    model = models.load_model(filepath='saved_models/transfer_learning_epoch_00.h5')

    TEST_STEPS = len(X_test)//BATCH_SIZE
    pred_probs = model.predict_generator(generator = batch_generator(X_test, BATCH_SIZE), 
                                       steps=TEST_STEPS)
